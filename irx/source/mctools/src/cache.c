...
static void DeinitPageCache(void)
{
    ...
    if (CacheInitialized)
    {
        ...
        {
            if (PageCache[i].buffer != NULL)
                free(PageCache[i].buffer);
        }
...
int FlushPageCache(void)
{
    ...
    if (CacheInitialized)
    {
        if ((FlushList = malloc(sizeof(struct CachePageFlushList) * CacheBlockSize)) == NULL)
            return -ENOMEM;
        ...
        for (BlockNum = 0; BlockNum < CacheCardSizeBlocks; BlockNum++)
        {
            ...
            for (i = 0; i < PAGE_CACHE_SIZE; i++)
            {
                if ((!(PageCache[i].flags & PAGE_CACHE_PAGE_EXISTS)) || (!(PageCache[i].flags & PAGE_CACHE_PAGE_IS_DIRTY)))
                    continue;
